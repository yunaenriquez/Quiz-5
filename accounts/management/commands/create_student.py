...
class Command(BaseCommand):
    help = 'Create a student user interactively'

    def add_arguments(self, parser):
        parser.add_argument(
            '--email',
            type=str,
            help='The email address of the student (optional, will prompt if not provided)',
        )
        parser.add_argument(
            '--first-name',
            type=str,
            help='The first name of the student (optional, will prompt if not provided)',
        )
        parser.add_argument(
            '--last-name',
            type=str,
            help='The last name of the student (optional, will prompt if not provided)',
        )
    ...
    def handle(self, *args, **options):
        ...
        try:
            student = CustomUser.objects.create_student(
                email=email,
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    f'Successfully created student: {student.get_full_name()} ({student.email})'
                )
            )
